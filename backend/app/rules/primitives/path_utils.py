"""
Пути к полям структуры признаков в JSON декларации (источник данных в условиях).

Разбор нотации «массив[*].поле», извлечение значений для условий по полю, таблице показателей,
межполевых правил. classification, cross_rules, compiler (пакет primitives).
"""
...
def extract_values(data: Any, path: str) -> List[Any]:
    ...
    # На каждом шаге пути накапливаем все значения, достижимые из корня данных.
    ...
    for step in steps:
        ...
        if step.kind == "prop":
            ...
            # Разворачиваем массив: каждый элемент становится отдельной веткой обхода.
            for value in current:
