"""
Вычисление выражения «Формула по нескольким показателям».

Только сложение, вычитание, умножение, деление и скобки над подставленными числами из строк
таблицы показателей; без произвольного кода. classification, analysis.overlap.
"""
...
def _eval_node(node: ast.AST, env: Dict[str, float]) -> float:
    """Рекурсивно вычисляет узел дерева разбора; допускаются только арифметика и имена из env."""
    ...
    if isinstance(node, ast.BinOp):
        ...
        # Деление на ноль — явная ошибка, как при ручном расчёте в правиле.
