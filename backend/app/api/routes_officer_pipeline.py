"""
Сквозная проверка таможенной декларации для интерфейса инспектора.

Координирует вызов сервиса предобработки (извлечение структурированных признаков LLM),
выбор основного справочника по ТН ВЭД, семантическую проверку кандидата класса,
правило-ориентированную классификацию и валидацию по схеме справочника (Pydantic).
Пояснения ошибок — на русском для экрана инспектора. Эндпоинт officer_run и вспомогательные.
"""
...
def _find_best_rule_for_tnved(
    ...
    for rule, rv in rows:
        # Код декларации должен начинаться с группы справочника (префикс ТН ВЭД).
    ...
    # Несколько справочников с одинаковой длиной префикса — смотрим «основной» каталог группы.
    grp_key = tier[0][3]
