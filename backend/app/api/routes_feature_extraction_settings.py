"""
REST API вкладки «Настройка справочников» интерфейса эксперта.

Хранение в PostgreSQL (AppSetting): конфигурации промптов и параметров моделей для
извлечения признаков по каждому справочнику; выбор основного справочника на код группы
ТН ВЭД; журнал прогонов подбора few-shot примеров из неразмеченного корпуса (APIE).
Те же ключи читают сервис предобработки и оркестратор.
"""
