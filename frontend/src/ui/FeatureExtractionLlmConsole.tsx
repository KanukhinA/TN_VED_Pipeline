...
export type LlmOperationLogState = {
  model: string;
  log: string;
  durationSec?: number;
  ok: boolean;
 ...
 */
export default function FeatureExtractionLlmConsole({
  mergedText,
  terminal,
  onRefreshLogs,
  storageKey = "fe_llm_console_expanded",
}: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ margin: 0, fontSize: 16, fontWeight: 600, color: "#0f172a" }}>Консоль модели</h2>
      ...
      {expanded ? (
        <pre
          ref={preRef}
          role="log"
          aria-label="Результат операций и логи контейнера с моделью"
          className="fe-llm-console__pre"
